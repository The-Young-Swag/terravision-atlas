...
export function StatusBar({ activeMode }: StatusBarProps) {
  ...
  return (
    <>
      {/* Footer width is sourced from the search bar shell's class token so
          the two stay in lockstep at every breakpoint — change one class
          and both reflow. */}
      <footer className={`search-bar-shell glass absolute bottom-4 left-4 z-10 flex flex-col gap-2 rounded-2xl px-4 py-2 font-mono text-[11px] sm:flex-row sm:items-center sm:justify-between ${isBrightBasemap ? 'text-slate-600' : 'text-slate-400'}`}>
