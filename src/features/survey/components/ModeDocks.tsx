...
export function ModeDocks({ activeMode }: ModeDocksProps) {
  ...
  const setMeasureMode = useSurveyStore((s) => s.setMeasureMode);
  const measureClosed = useSurveyStore((s) => s.measureClosed);
  const setMeasureClosed = useSurveyStore((s) => s.setMeasureClosed);
  const setMeasureActive = useSurveyStore((s) => s.setMeasureActive);
  const clearMeasure = useSurveyStore((s) => s.clearMeasure);
